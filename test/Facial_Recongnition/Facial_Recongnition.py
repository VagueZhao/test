...
classfier=cv2.CascadeClassifier("D:/python3.6.3/Lib/site-packages/opencv-master/data/haarcascades/haarcascade_frontalface_alt.xml")#定义分类器
while success:
  success, frame = cap.read()
  size=frame.shape[:2]#获得当前帧彩色图像的大小
  ...
  if len(faceRects)>0:#如果人脸数组长度大于0
   for faceRect in faceRects:#对每一个人脸画矩形框
    ...
    c = chr(key & 255)
    # 写入图像
    cv2.imwrite('F:/test/images/faceImg.jpg', image)
   # if c in ['q', 'Q', chr(27)]:
       # break
